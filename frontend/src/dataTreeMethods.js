...
function findLastCommonIndex(arrays) {
    let lastIndex = -1;
    let minLength = Math.min(...arrays.map(arr => arr.flow.length));

    for (let i = 0; i < minLength; i++) {
        let currentKeys = arrays.map(arr => arr.flow[i].link);
...
function mergeDataFlows(dataFlows, mergedSourceKey, key) {
    if (dataFlows.length == 1) return [...dataFlows[0].flow];
    if (dataFlows.length == 0 || dataFlows.every(referencePoints => referencePoints.flow.length == 0)) return [];
    const commonObjects = [];
    let mergedData = {};

    const minLength = Math.min(...dataFlows.map(referencePoints => referencePoints.flow.length));
    const lastCommonIndex = findLastCommonIndex(dataFlows)
    for (let i = 0; i < minLength; i++) {
        const referencePoints = dataFlows.map(referencePoints => referencePoints.flow[i]);
...
export function collectInputFlows(dataTree, startNodeId, memo = {}) {
    function recurse(nodeId) {
        const node = dataTree[nodeId];
        const flows = node.parents.map(parent => {
            if (memo[parent.id]) {
                return memo[parent.id];
            }
            const flows = recurse(parent.id);
            const previousParentsFlow = mergeDataFlows(flows, parent.id, parent.sourceKey);
            const parentFlow = loadNodeToFlow(dataTree, previousParentsFlow, parent.id, parent.sourceKey)

            memo[parent.id] = { edgeId: parent.edgeId, flow: parentFlow };
            return { edgeId: parent.edgeId, flow: parentFlow };
        });
...
export function collectAvailableDataFrom(dataTree, startNodeId, sourceKey, memo) {
    if (!startNodeId) return {};
    const dataFlows = collectInputFlows(dataTree, startNodeId)
    if (dataFlows.length == 0) return {};
    const previousDataFlows = mergeDataFlows(dataFlows, startNodeId, sourceKey, memo);
...
export function collectAvailableDataOn(dataTree, startNodeId, targetKey, memo) {
    if (!startNodeId) return {};
    const dataFlows = collectInputFlows(dataTree, startNodeId, memo)
    let dataFlow = {}
    if (dataFlows.length == 0) return {};
    if (dataFlows.length == 1) {
        dataFlow = dataFlows[0].flow
    } else {
        dataFlow = mergeDataFlows(dataFlows, startNodeId, targetKey);
    }
    const dataLinksByTypes = flatDataFlow(dataFlow);
    return dataLinksByTypes;
}
...
export function getBranchUpdate(dataTree, rootId) {
    ...
    const memo = {};
    const updates = [];
    const data = collectAvailableDataOn(dataTree, rootId, 'target', memo)
    ...
    const childrenUpdates = getChildrenUpdate(dataTree, rootId, memo)
    return {
        usedKeys: [...updates, ...childrenUpdates.usedKeys],
        flow: Object.values(memo).map(flow => getFlow(flow))
    }
}

export function getChildrenUpdate(dataTree, rootId, memo = {}) {
    if (!rootId) return {};
    const updates = [];
    ...
    function recurse(nodeId) {
        ...
        node.children.forEach(child => {
            if (memo[child.id]) return;
            const data = collectAvailableDataOn(dataTree, child.id, 'target', memo)
